...
class SelfAttention(nn.Module):
    ...
    def forward(self, obs, predator_features, searcher_features, batch_size):
        ...
        for b in range(batch_size):
            ...
            if nan_mask.any():
                weights.masked_fill(nan_mask, 0.0)  # 将 NaN 设置为 0
            ...
            no_searcher_mask = (weights.sum(dim=-1) == 0).unsqueeze(1).expand(-1, self.hidden_dim)  # shape: (N,)
            new_features = new_features * (~no_searcher_mask).float() + predator_features[b] * no_searcher_mask.float()
