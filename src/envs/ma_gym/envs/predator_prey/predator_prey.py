...
class PredatorPrey(gym.Env):
    ...
    def __init__(self, grid_shape=(5, 5), n_agents=2, n_preys=1, prey_move_probs=(0.175, 0.175, 0.175, 0.175, 0.3),
        ...
        self.obs_dim = 2 * (n_agents + n_preys)
        self.observation_space = MultiAgentObservationSpace(
            [spaces.Box(low=-np.inf, high=+np.inf, shape=(self.obs_dim,), dtype=np.float32) for _ in range(self.n_agents)])

        self.share_obs_dim = self.n_agents * self.observation_space[0].shape[0]
        self.share_observation_space = [spaces.Box(
            low=-np.inf, high=+np.inf, shape=(self.share_obs_dim,), dtype=np.float32) for _ in self.observation_space]
                ...
                for row in range(0, self._grid_shape[0]):
                    for col in range(0, self._grid_shape[1]):
                        if PRE_IDS['prey'] in self._full_obs[row][col]:
                            _prey_pos += [row - pos[0], col - pos[1]]
                        if PRE_IDS['agent'] in self._full_obs[row][col] and pos != [row, col]:
                            _agent_pos += [row - pos[0], col - pos[1]]
            else:
                for row in range(max(0, pos[0] - self.obs_range), min(pos[0] + self.obs_range + 1, self._grid_shape[0])):
                    for col in range(max(0, pos[1] - self.obs_range), min(pos[1] + self.obs_range + 1, self._grid_shape[1])):
                        if PRE_IDS['prey'] in self._full_obs[row][col]:
                            _prey_pos += [row - pos[0], col - pos[1]]
                        if PRE_IDS['agent'] in self._full_obs[row][col] and pos != [row, col]:
                            _agent_pos += [row - pos[0], col - pos[1]]
            while len(_prey_pos) < 2 * self.n_preys:
                _prey_pos += [0, 0]
            while len(_agent_pos) < 2 * (self.n_agents - 1):
                _agent_pos += [0, 0]
            _agent_i_obs += _agent_pos
    ...
    def searcher_comm_reward(self, s_i):
        pos = self.agent_pos[s_i]
        n_p = 0
        for row in range(max(0, pos[0] - self.comm_range), min(pos[0] + self.comm_range + 1, self._grid_shape[0])):
            for col in range(max(0, pos[1] - self.comm_range), min(pos[1] + self.comm_range + 1, self._grid_shape[1])):
                if PRE_IDS['agent'] in self._full_obs[row][col]:
                    if int(self._full_obs[row][col].split(PRE_IDS['agent'])[1]) <= self.n_predators:
                        n_p += 1
        return min(n_p, 2)
    ...
    def step(self, agents_action):
        ...
        for s_i in range(self.n_predators, self.n_agents):
            _reward = 0.1 * self.searcher_comm_reward(s_i)
            for agent_i in range(self.n_agents):
                rewards[agent_i] += _reward
            # for agent_i in range(self.n_predators, self.n_agents):
            #     rewards[agent_i] += _reward

        for prey_i in range(self.n_preys):
            if self._prey_alive[prey_i]:
                prey_pos = self.prey_pos[prey_i]
                dis = []
                _reward = 0
                for p_i in range(self.n_predators):
                    p_pos = self.agent_pos[p_i]
                    dis.append(np.sqrt(np.square(prey_pos[0] - p_pos[0]) + np.square(prey_pos[1] - p_pos[1])))
                for min2 in heapq.nsmallest(2, dis):
                    _reward -= 0.1 * min2
                for agent_i in range(self.n_agents):
                    rewards[agent_i] += _reward
                ...
                for i in n_i:
                    if i >= self.n_predators:
                        predator_neighbour_count -= 1
                if predator_neighbour_count >= 1:
                    _reward = self._prey_capture_reward if predator_neighbour_count > 1 else 0
    ...
    def render(self, mode='human'):
        ...
        for agent_i in range(self.n_predators, self.n_agents):
            draw_circle(img, self.agent_pos[agent_i], cell_size=CELL_SIZE, fill=SREARCER_COLOR)
            write_cell_text(img, text=str(agent_i + 1), pos=self.agent_pos[agent_i], cell_size=CELL_SIZE,
                            fill='white', margin=0.4)
    ...
    def get_env_info(self):
        env_info = {"n_agents": self.n_agents, "n_actions": 5, "state_shape": self.share_obs_dim,
                    "obs_shape": self.obs_dim, "episode_limit": self.episode_limit}
        return env_info
    ...
    def get_avail_actions(self):
        avail_actions = np.zeros((self.n_agents, 5))
        for agent_i in range(self.n_agents):
